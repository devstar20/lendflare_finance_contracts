...
contract ConvexBooster is Initializable, ReentrancyGuard {
    ...
    function addConvexPool(uint256 _originConvexPid) public onlyOwner {
        (
            address lpToken,
            ,
            ,
    ...
    function updateExtraRewards(uint256 _pid) public onlyOwner {
        ...
        (, , , address originCrvRewards, , bool shutdown) = IConvexBooster(
            convexBooster
        ...
        if (originExtraRewardsLength > currentExtraRewardsLength) {
            for (
    ...
    function liquidate(
        ...
        address _user,
        uint256 _amount
    ) external onlyLendingMarket returns (address, uint256) {
        PoolInfo storage pool = poolInfo[_pid];
